...
- The user may run `vp run dev:all` in a separate tmux session. Do not run long-lived dev watchers unless asked.
...
The site is static HTML (no backend). Source is in `../sitegeist-site/src/frontend/`.
